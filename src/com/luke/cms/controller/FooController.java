 ...
 * Spring 用例
 * 
 * @Component @Service 等默认都是 @Scope("singleton") 除非特别声明为 @Scope("prototype")
 *            否则都是以单例方式注入
 */
@Controller
@RequestMapping("/test/*")
...
public class FooController {
	...
	public StudentList resultFormatByAccept() {
		...
		list.add(new Student(3, "Robert", "Parera", "robert@gmail.com", "8978767878"));
		list.add(new Student(93, "Andrew", "Strauss", "andrew@gmail.com", "8978767878"));
		list.add(new Student(239, "Eddy", "Knight", "knight@gmail.com", "7978767878"));
	...
	public ArrayList<FooBean> paramChinese(
			...
			@RequestParam(value = "chinese_name", required = false, defaultValue = "无名") String chinese,
			ModelMap model) {
	...
	@ResponseBody
	public Result handleFormUpload(@RequestParam("name") String name, @RequestParam("file") MultipartFile file) {
		try {
			if (!file.isEmpty()) {
//				byte[] bytes = file.getBytes();
				File localfile = new File("/home/luke/"+name);
				localfile.deleteOnExit();
				localfile.createNewFile();
				file.transferTo(localfile);
				// store the bytes somewhere
				return new Success("your upload file name:"+name+" size:"+file.getSize(), null);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		return new Failed("upload file error");
	}
	...
	public void downloadCSV(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			response.getOutputStream().write(fooService.toCSV(titles, contentList).getBytes("utf-8"));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public void tinyImage(HttpServletResponse response) throws IOException {
		final byte[] whiteSpot = new byte[] { 71, 73, 70, 56, 57, 97, 1, 0, 1, 0, -128, -1, 0, -1, -1, -1, 0, 0, 0, 44,
